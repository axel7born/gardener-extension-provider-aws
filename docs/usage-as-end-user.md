...
The `enableDualstack` flag specifies whether dualstack or IPv4 only should be supported by the infrastructure.
When the flag is set to true an amazon provided IPv6 CIDR block will be attached to the VPC and all subnets and a route entry is added to the main route table to route all IPv6 traffic over the IGW. 
